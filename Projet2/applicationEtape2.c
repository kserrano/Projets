...
// * Auteur : LSR
// * Date : 2013
// * Goal : Application producteur-consommateur for step 2 of FPGA project
...
void deposer(int t, int k) {
	...
	while (nbElement[t] == TAILLETAMPON) {
		timedAwait(numCondition[numVerrou[t]][1],0);
	}
	...
	signalAll(numCondition[numVerrou[t]][0]);
...
int prelever(int t) {
	...
	//Enter the lock with the id numVerrou[t]
	verrouiller(numVerrou[t]);
	//Wait for a signal if the buffer t is empty or the timeout is not passed
	while ((nbElement[t] == 0) && (to != 0)) {
		to = timedAwait(numCondition[numVerrou[t]][0], 1000);
	}

	if (to == 0) // if timeout passed
	{
		//Exit the lock with the id numVerrou[t] and return -1
		deverrouiller(numVerrou[t]);
		return (-1);
	} else // if an element is inserted in the buffer t
	{
		...
		signalAll(numCondition[numVerrou[t]][1]);
		//Exit the lock with the id numVerrou[t]
		deverrouiller(numVerrou[t]);
...
void consommateur0(void) {
	...
	while (1) {
		...
		//turn on the first bit of led 0 for specified duration, depending on whether an item received or a timeout passed
		if (item == -1) {
			while (i < DELAYONTIMEOUT) {
				i++;
			}
		} else {
			while (i < DELAYON) {
...
void consommateur1(void) {
	...
	while (1) {
		...
		//turn on the first bit of led 1 for specified duration, depending on whether an item received or a timeout passed
		if (item == -1){
			while (i < DELAYONTIMEOUT) {
				i++;
			}
...
void consommateur2(void) {
	...
	while (1) {
		...
		//turn on the first bit of led 2 for specified duration, depending on whether an item received or a timeout passed
		if (item == -1){
			while (i < DELAYONTIMEOUT) {
				i++;
			}
